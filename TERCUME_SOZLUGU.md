...
**AR Faz 2 part 3 (v1180, Faz 6.15) — accessor genelleştirme + ASIL ARAPÇA tablolar:** aşağıdaki accessor'lar
`S.lang==='en'`→çok-dilli (`ar` branch) genelleştirildi ve `*_AR` tabloları eklendi:
- `STORY_AR` (ada açılış sîret satırları — MSA prose) · `storyEn` ar branch.
- `NPC_AR` (muhtaç ikram görevleri + hadîs ASIL Arapça: yetim «أنا وكافل اليتيم...» / komşu «ما زال جبريل يوصيني بالجار...»
  / misafir «من كان يؤمن بالله واليوم الآخر فليكرم ضيفه») · `npcNm`/`npcRef`.
- `ZIKIR_AR` + `TESBIHAT_AR` (zikir ASIL Arapça lafzı: hamdele/salvele/istiğfâr/esmâ + tesbîhât) · `zikirAd`/`zikirTxt` + `__zikir` tesbihat.
- `IMAM_SOZ_AR` (6 hadîs ASIL Arapça: الدين النصيحة / نعمتان مغبون فيهما / يسّروا ولا تعسّروا / خيركم من تعلّم القرآن / المرء مع من أحبّ / المسلم من سلم المسلمون) · `imamSoz`.
- `obligationsText` ar branch (mükellefiyet metni MSA).
- `QBANK_AR` (imtihan bankası — 16 konu, QBANK ile aynı sıra→c=0 korunur; âyet/hadîs iktibasları ASIL Arapça, soru gövdesi MSA)
  · `buildExam` `_loc` (en/ar) genelleştirildi. Parite doğrulandı (QB=AR=EN=16, tüm şık sayıları eşit) + preview'da AR imtihan render teyit.
> **UZMAN TEYİDİ:** QBANK_AR/NPC_AR/IMAM_SOZ_AR/ZIKIR_AR hadîs lafızları meşhur muteber rivayetlerdir; âyet iktibasları mushaf lafzı.
> Matbu külliyat + tam tahriç (kitap/bab no) uzman teyidine tabidir.

**KALAN AR (sonraki):** görev-sheet ref'leri (`refs` alanları), Umre/hac ritüel metinleri, zekât nisab/hatırlatma prose,
hayır panel dinî ref'leri — kalan en-only string'ler; AR için genelleştir + asıl Arapça. Ayrıca Mescid panel body m_* AR anahtarları eksik.
